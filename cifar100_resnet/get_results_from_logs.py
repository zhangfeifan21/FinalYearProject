...
parser_cmd = argparse.ArgumentParser(description='PyTorch CIFAR100 test')
parser_cmd.add_argument('--batch-size', type=int, default=32, metavar='N',
                        help='input batch size for training (default: 32)')
parser_cmd.add_argument('--test-batch-size', type=int, default=32, metavar='N',
                        help='input batch size for testing (default: 32)')
parser_cmd.add_argument('--epochs', type=int, default=300, metavar='N',
                        help='number of epochs to train (default: 300)')
parser_cmd.add_argument('--lr', type=float, default=0.1, metavar='LR',
                        help='learning rate (default: 0.1)')
parser_cmd.add_argument('--momentum', type=float, default=0.9, metavar='M',
                        help='SGD momentum (default: 0.9)')
parser_cmd.add_argument('--no-cuda', action='store', default=False,
                        help='disables CUDA training')
parser_cmd.add_argument('--seed', type=int, default=17, metavar='S',
                        help='random seed (default: 17)')
parser_cmd.add_argument('--log-interval', type=int, default=100, metavar='N',
                        help='how many batches to wait before logging training status')
parser_cmd.add_argument('--optimizer', type=str, default='adam',
...
parser_cmd.add_argument('--data', type=str, default='cifar100')
...
parser_cmd.add_argument('--decay-schedule', type=str, default='cosine',
...
def calc_and_print_data(args, accs, losses, arg):
    ...
    print('Arguments:\n{0}\n'.format(arg))
    print(
        'Accuracy. Median: {5:.5f}, Mean: {0:.5f}, Standard Error: {1:.5f}, Sample size: {2}, 95% CI: ({3:.5f},{4:.5f})'.format(
            np.mean(accs), acc_se, len(accs),
            np.mean(accs) - (1.96 * acc_se), np.mean(accs) + (1.96 * acc_se), np.median(accs)))
    print(
        'Error.    Median: {5:.5f}, Mean: {0:.5f}, Standard Error: {1:.5f}, Sample size: {2}, 95% CI: ({3:.5f},{4:.5f})'.format(
            1.0 - np.mean(accs), acc_se, len(accs),
            (1.0 - np.mean(accs)) - (1.96 * acc_se), (1.0 - np.mean(accs)) + (1.96 * acc_se),
            1.0 - np.median(accs)))
    print(
        'Loss.     Median: {5:.5f}, Mean: {0:.5f}, Standard Error: {1:.5f}, Sample size: {2}, 95% CI: ({3:.5f},{4:.5f})'.format(
            np.mean(losses), loss_se, len(losses),
...
for folder in folders:
    losses = []
    accs = []
    for log_name in glob.iglob(join(folder, '*.log')):  # 遍历文件，支持通配符
            ...
            for line in f:
                if 'Namespace' in line:
                    ...
                    if args.agg_config:
                        ...
                        arg_str = shlex.split(arg)  # 去除双引号的分割
                ...
                if skip:
                    continue
                if not line.startswith('Test evaluation'):
                    continue
                try:
