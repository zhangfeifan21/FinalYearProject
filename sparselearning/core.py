...
def add_sparse_args(parser):
    parser.add_argument('--growth', type=str, default='momentum',
    ...
    parser.add_argument('--redistribution', type=str, default='momentum',
...
class Masking(object):
    ...
    def truncate_weights(self):
        ...
        self.adjusted_growth = 0.25 * self.adjusted_growth + (
                    0.75 * (self.baseline_nonzero - total_nonzero_new)) + np.mean(self.adjustments)
    ...
    def get_gradient_for_weight(self, weight):
        ...
        for module in self.modules:
            for name, tensor in module.named_parameters():
                ...
                if name in self.name2variance:
                    val = '{0}: {1}->{2}, density: {3:.3f}, proportion: {4:.4f}'.format(name, self.name2nonzeros[name],
                                                                                        num_nonzeros,
                                                                                        num_nonzeros / float(
                                                                                            mask.numel()),
